...
export default function NetworkFilter() {
  ...
  return (
    <StyledMenu ref={node}>
      ...
      {open && (
        <MenuTimeFlyout>
          {BACKEND_CHAIN_NAMES.map((network, index) => {
            const chainInfo = getChainInfo(CHAIN_NAME_TO_CHAIN_ID[network])
            if (!chainInfo || index !== 0 ) return null
            return (
          ...
          <InternalLinkMenuItem data-testid="tokens-network-filter-option-bnb-chain">
            <NetworkLabel>
              <Logo src={OmchainLogo} />
              Omchain
            </NetworkLabel>
          </InternalLinkMenuItem>
        </MenuTimeFlyout>
